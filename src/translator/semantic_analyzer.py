...
class SemanticAnalyzer:
    ...
    def analyze(self):
        ...
        if 'start' not in self.labels:
            raise SemanticError("No entry point found in provided file (label 'start' is missing)")
    ...
    def check_section(self, section):
        if section.section_type not in ['text', 'data', 'devices']:
            raise SemanticError(f"Invalid section type: {section.section_type}")
    ...
    def check_instruction(self, instruction):
        ...
        if instruction.opcode not in ['byte', 'char', 'res', 'str'] and self.current_section == 'data':
            raise SemanticError(f"Instruction {instruction.opcode} is not allowed in data section")

        if instruction.opcode not in ['byte', 'addr'] and self.current_section == 'devices':
            raise SemanticError(f"Instruction {instruction.opcode} is not allowed in devices section")

        if instruction.opcode in ['nop', 'pop', 'pushf', 'popf', 'inc', 'dec', 'swap', 'dup', 'nop', 'ret', 'halt',
                                  'iret', 'ei', 'di', 'in', 'out', 'not', 'neg', 'shl', 'shr', 'rol', 'ror', 'add',
                                  'sub', 'mul', 'div', 'and', 'or', 'xor', 'ld', 'st', 'cmp']:
            if instruction.operands:
                raise SemanticError(f"Instruction {instruction.opcode} does not take any operands")

        if instruction.opcode in ['push', 'call', 'int', 'jmp', 'call', 'jz', 'je', 'jnz', 'jg', 'jge', 'jl', 'jle',
                                  'str']:
            if len(instruction.operands) != 1:
                raise SemanticError(f"Instruction {instruction.opcode} takes exactly one operand")

        if instruction.opcode in ['jmp', 'call', 'jz', 'je', 'jnz', 'jg', 'jge', 'jl', 'jle']:
            if instruction.operands[0] not in self.labels:
                raise SemanticError(f"Label {instruction.operands[0]} not found")

        if instruction.opcode == 'byte':
            if len(instruction.operands) == 0:
                raise SemanticError("Instruction byte requires at least one operand")
            for operand in instruction.operands:
                if not isinstance(operand, int):
                    raise SemanticError(f"Operand {operand} is not a number")
                if operand < 0 or operand > 255:
                    raise SemanticError(f"Operand {operand} is out of range for byte")

        if instruction.opcode == 'char':
            if len(instruction.operands) == 0:
                raise SemanticError("Instruction char requires at least one operand")
            for operand in instruction.operands:
                if not isinstance(operand, str):
                    raise SemanticError(f"Operand {operand} is not a string")
                if len(operand) != 1:
                    raise SemanticError(f"Operand {operand} is not a character")

        if instruction.opcode == 'str':
            if len(instruction.operands) == 0:
                raise SemanticError("Instruction str requires at least one operand")
            for operand in instruction.operands:
                if not isinstance(operand, str):
                    raise SemanticError(f"Operand {operand} is not a string")
                for char in operand:
                    if ord(char) < 0 or ord(char) > 255:
                        raise SemanticError(f"Character {char} is out of range for byte")
        ...
        if instruction.opcode == 'res':
            ...
            if not isinstance(instruction.operands[0], int):
                raise SemanticError(f"Operand {instruction.operands[0]} is not a number")
            if instruction.operands[0] < 1:
                raise SemanticError(f"Operand {instruction.operands[0]} should be greater than 0")
